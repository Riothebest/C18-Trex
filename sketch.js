...
function setup() {
  ...
  console.log("window hight: ",windowHeight);
...
function draw() {
  ...
  if (gameState===PLAY)
  {
    ...
    //initially the touches array there is no element
    if( touches.length >0 ||keyDown("SPACE") && trex.y >= height-110) 
    {
      jumpSound.play( )
      trex.velocityY = -15;

      //empty the array --> make sure that there is no element inside the array
      touches = [];
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(width,173,40,10);
    cloud.y = Math.round(random(10,260));
